<template>


  <footer class="bg-opacity text-black text-sm">

    <div class="w-full p-4 flex justify-between px-8">
      <div class="flex-row flex items-center text-left">
        <img src="../assets/images/Moveout_Logo2.svg" alt="Moveout Logo" class="h-12 w-auto" />
        <p class="ml-3">Moveout Ltd.</p>
        <p class="ml-6">Helping you find a home since 2022</p>
      </div>

      <ul class="flex flex-nowrap items-center mt-3 text-sm font-semibold sm:mt-0 ">
        <!-- <li>
          <a href="/aboutus" class="hover:underline me-4">About</a>
        </li>
        <li>
          <a href="/contactus" class="hover:underline me-4">Contact</a>
        </li> -->
        <li class="content-center items-center me-4" >
          <a href="https://www.instagram.com/moveout.ai/?igsh=MTI1amg1amk2YzV6cw%3D%3D&utm_source=qr" target="_blank">
            <svg fill="black" width="25px" height="25px" viewBox="0 0 256 256" id="Flat" xmlns="http://www.w3.org/2000/svg">
              <path d="M128,82a46,46,0,1,0,46,46A46.05239,46.05239,0,0,0,128,82Zm0,68a22,22,0,1,1,22-22A22.02489,22.02489,0,0,1,128,150ZM176,20H80A60.06812,60.06812,0,0,0,20,80v96a60.06812,60.06812,0,0,0,60,60h96a60.06812,60.06812,0,0,0,60-60V80A60.06812,60.06812,0,0,0,176,20Zm36,156a36.04061,36.04061,0,0,1-36,36H80a36.04061,36.04061,0,0,1-36-36V80A36.04061,36.04061,0,0,1,80,44h96a36.04061,36.04061,0,0,1,36,36ZM196,76a16,16,0,1,1-16-16A16.01833,16.01833,0,0,1,196,76Z"/>
            </svg>
          </a>
        </li>
        <li class="content-center items-center">
          <a href="https://www.tiktok.com/@moveout_mtl?lang=en" target="_blank">
            <svg fill="black" width="25px" height="25px" viewBox="0 0 512 512" id="icons" xmlns="http://www.w3.org/2000/svg"><path d="M412.19,118.66a109.27,109.27,0,0,1-9.45-5.5,132.87,132.87,0,0,1-24.27-20.62c-18.1-20.71-24.86-41.72-27.35-56.43h.1C349.14,23.9,350,16,350.13,16H267.69V334.78c0,4.28,0,8.51-.18,12.69,0,.52-.05,1-.08,1.56,0,.23,0,.47-.05.71,0,.06,0,.12,0,.18a70,70,0,0,1-35.22,55.56,68.8,68.8,0,0,1-34.11,9c-38.41,0-69.54-31.32-69.54-70s31.13-70,69.54-70a68.9,68.9,0,0,1,21.41,3.39l.1-83.94a153.14,153.14,0,0,0-118,34.52,161.79,161.79,0,0,0-35.3,43.53c-3.48,6-16.61,30.11-18.2,69.24-1,22.21,5.67,45.22,8.85,54.73v.2c2,5.6,9.75,24.71,22.38,40.82A167.53,167.53,0,0,0,115,470.66v-.2l.2.2C155.11,497.78,199.36,496,199.36,496c7.66-.31,33.32,0,62.46-13.81,32.32-15.31,50.72-38.12,50.72-38.12a158.46,158.46,0,0,0,27.64-45.93c7.46-19.61,9.95-43.13,9.95-52.53V176.49c1,.6,14.32,9.41,14.32,9.41s19.19,12.3,49.13,20.31c21.48,5.7,50.42,6.9,50.42,6.9V131.27C453.86,132.37,433.27,129.17,412.19,118.66Z"/></svg>
          </a>
        </li>
      </ul>
    </div>
  </footer>
</template>

<style scoped>
/* Additional scoped styles if needed */
</style>
